...
tf.flags.DEFINE_string("data_dir", "/data/vllab1/dataset/CITYSCAPES/CITY/human_new_image", "path to dataset")
tf.flags.DEFINE_string("label_dir", "/data/vllab1/dataset/CITYSCAPES/CITY/human_new_mask", "path to annotation")
tf.flags.DEFINE_string("model_dir", "/data/vllab1/checkpoint/context inpainting/instance/", "path to model directory")
...
images_hiding = tf.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.image_size_h, FLAGS.image_size_w, 3], name='images_hiding')
...
for epoch in range(n_epochs):
    for batch_itr in xrange(train_size / FLAGS.batch_size):
        ...
        train_images = np.array([scipy.misc.imread(train_image_name).astype(np.float32)
                                for train_image_name in train_images_name])

        train_annotations = np.array([read_mask(train_annotation_name)
                                for train_annotation_name in train_annotations_name])

        train_images = np.array(train_images) / 255 * 2 - 1
        #train_crops = [train_image[32:160, 128:384, :]
        ...
        _, loss_G_val, adv_pos_val, adv_neg_val, loss_recon_val, loss_adv_G_val, reconstruction_vals, recon_ori_vals, bn1_val,bn2_val,bn3_val,bn4_val,bn5_val,bn6_val,debn4_val, debn3_val, debn2_val, debn1_val = sess.run(
                ...
                feed_dict={
                    ...
                    images_hiding: train_images,
                    images_mask: train_annotations,
    ...
    if epoch % 40 == 0:
        ...
        sample_annotations = (1-train_annotations) * samples
        reconstruction_vals = (255. * (reconstruction_vals + 1) / 2.).astype(int)
        reconstruction_vals = train_annotations * reconstruction_vals + sample_annotations
        if epoch == 0:
            ...
            scipy.misc.imsave(os.path.join(FLAGS.result_dir, '{:d}_ori_mask.png'.format(epoch))
                              , merge(sample_annotations, (FLAGS.sample_shape, FLAGS.sample_shape)))
