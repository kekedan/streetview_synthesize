...
class Model():
    ...
    def build_reconstruction( self, images, is_train):
        ...
        with tf.variable_scope('GEN'):
            ...
            deconv0 = self.new_deconv_layer( debn1, [4,4,64,64], conv1.get_shape().as_list(), stride=2, name="deconv0")
            debn0 = tf.nn.relu(self.batchnorm(deconv0, is_train, name='debn0'))

            #recon = self.new_deconv_layer( debn1, [4,4,3,64], [batch_size,128,256,3], stride=2, name="recon")
            recon = self.new_deconv_layer(debn0, [4, 4, 3, 64], [batch_size, 256, 512, 3], stride=2, name="recon")

        return bn1, bn2, bn3, bn4, bn5, bn6, debn4, debn3, debn2, debn1, recon, tf.nn.tanh(recon)

    def build_heatmap( self, images, is_train):
        batch_size = images.get_shape().as_list()[0]
        ...
        with tf.variable_scope('GEN'):
            ...
            recon = self.new_deconv_layer(debn0, [4, 4, 2, 64], [batch_size, 256, 512, 2], stride=2, name="recon")

        return bn1, bn2, bn3, bn4, bn5, bn6, debn4, debn3, debn2, debn1, recon, tf.nn.tanh(recon)

    def build_adversarial(self, images, is_train, reuse=None):
        with tf.variable_scope('DIS', reuse=reuse):
            conv1 = self.new_conv_layer(images, [4,4,2,64], stride=2, name="conv1" )
            bn1 = self.leaky_relu(self.batchnorm(conv1, is_train, name='bn1'))
